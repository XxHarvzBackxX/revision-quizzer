import { ArrowLeft, ArrowRight, BookOpen, Check, CheckCircle2, Clock3, Gamepad2, Highlighter, Sparkles } from 'lucide-react';
            ...
            <div className="revision-page-cards">
              {course.pages.filter((page) => page.domainId === domain.id).map((page, index) => {
                ...
                return <article className={complete ? 'reviewed' : ''} key={page.id}>
                  <button className="review-page-toggle" onClick={() => { toggleReviewedPage(course.examCode, page.id); if (!complete) recordGuideReviewed(course.examCode, page.id); }} aria-label={complete ? `Mark ${page.title} not reviewed` : `Mark ${page.title} reviewed`}>{complete ? <Check size={16} /> : index + 1}</button>
