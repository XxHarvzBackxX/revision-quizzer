...
import { officialDatasetsFirst, validateDataset, type DatasetInput, type DatasetSummary, type PublicDataset } from '../shared/quiz.js';

const CREATED_AT = '2026-07-15T00:00:00.000Z';
const inputs = [
  { input: officialPractice, label: 'AI-901 official practice assessment', id: 'builtin-ai901-official-practice-assessment', slug: 'ai-901-official-microsoft-learn-practice-assessment', official: true },
  ...[paper1, paper2, paper3].map((input, index) => ({ input, label: `AI-901 paper ${index + 1}`, id: `builtin-ai901-paper-${index + 1}`, slug: `ai-901-mock-exam-${index + 1}`, official: false })),
  ...[azurePaper1, azurePaper2, azurePaper3].map((input, index) => ({ input, label: `AZ-900 paper ${index + 1}`, id: `builtin-az900-paper-${index + 1}`, slug: `az-900-mock-exam-${index + 1}`, official: false }))
].map(({ input, label, id, slug, official }) => ({
...
export const curatedDatasets: PublicDataset[] = inputs.map(({ dataset, id, slug, official }) => ({
  ...dataset,
  id,
  slug,
...
export function getCuratedSummaries(): DatasetSummary[] {
  return officialDatasetsFirst(curatedDatasets.map(({ items: _items, ...summary }) => summary));
}
